...
def plot_skeleton_kpts(im, kpts, radius=5, shape=(640, 640), confi=0.5, line_thick=1):
    ...
    limb_color = pose_palette[[9, 9, 9, 9, 7, 7, 7,
                               0, 0, 0, 0, 0, 16, 16, 16, 16, 16, 16, 16]]
    kpt_color = pose_palette[[16, 16, 16, 16,
    ...
    for i, k in enumerate(kpts):
        ...
        if x_coord % shape[1] != 0 and y_coord % shape[0] != 0:
            if len(k) == 3:
                ...
                if conf < confi:
                    continue
            cv2.circle(im, (int(x_coord), int(y_coord)), radius,
    ...
    for i, sk in enumerate(skeleton):
        ...
        if pos2[0] % shape[1] == 0 or pos2[1] % shape[0] == 0 or pos2[0] < 0 or pos2[1] < 0:
            continue
        cv2.line(im, pos1, pos2, [int(x) for x in limb_color[i]],
...
def norm_kpts(lm_list, torso_size_multiplier=2.5):
    ...
    try:
        pre_lm = list(np.array(
            [[(landmark[0]-center_x)/max_distance, (landmark[1]-center_y)/max_distance]
...
def plot_one_box(x, img, pose_class, label=None, line_thickness=3):

    if pose_class == "shoplifter":
        color = (0, 0, 255)
    elif pose_class == "pickup":
        color = (255, 0, 0)
    else:
        color = (0, 255, 0)
